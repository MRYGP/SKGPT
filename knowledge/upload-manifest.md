...
> 版本：v0.2 · 2026-04-30
> 变更：同步 013/014 复盘后的 Project 上传包刷新要求
...
## 五、2026-04-30 上传包刷新要求

由于 `MRYGP/SK` 已升级内容生产规则，以下文件须**重新**从主仓库 `main` 分支导出，并上传/替换到 ChatGPT Project 知识库（与本地缓存旧版混用会导致仍按旧模板写作）：

1. `content/article_template.md`  
   - 已含全局 `## 先说结论`、类型 G 机会招募型、类型 H 真实产品体检、时间变量推敲等硬规则。

2. `content/公众号内容生产经验手册.md`  
   - 已含教训 34、35、36 等本轮修订。

3. `core/product-teardown-template.md`  
   - 已升级到 v4.5，含 9.5 时间变量推敲。

4. `meta/CLAUDE系统指令.md`  
   - 若主仓库内已同步本轮写作规则，Project 中旧版亦应刷新，避免与 Project Instructions 打架。

如 Project 仍保留上述文件的旧版本，模型可能继续按旧写法做判断与过稿。
...
- 若未来约 1 个月**高频**处理「朋友产品 / 创业者产品 / 真实早期产品体检」，建议加入 Project 上传包。
- 若**偶尔**使用，可不占稳定内核名额；任务时通过 GitHub 现读或临时上传该次会话。

**配额提示**：

若加入，优先用**任务机动位**替换，不挤占 18 个稳定内核位（见上文「二、推荐配额」）。

**可选替换方案**（视近期任务频率）：

- **移出**：`meta/SK知识库使用方法-框架红队法.md`
- **移入**：`core/SKILL-真实产品外部体检与机会推演SOP.md`
- **前提**：若近期红队法使用低频，而真实产品体检高频。

**注意**：本节仅为上传策略建议；**不**将 `MRYGP/SK` 中动态状态类文件加入长期上传清单（见「一、总原则」）。
